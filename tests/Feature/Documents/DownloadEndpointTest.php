...
beforeEach(function () {
    ...
    $mock->shouldReceive('generatePresignedDownloadUrl')
        ->andReturn('https://s3.example/x?signed=1');
    app()->instance(CloudStorageService::class, $mock);
});

it('redirects an authed business member to the signed URL', function () {
    $response = $this->actingAs($this->member)
        ->get("/documents/{$this->document->ulid}/download");

    $response->assertRedirect('https://s3.example/x?signed=1');
});

it('redirects guests to /login', function () {
    $response = $this->get("/documents/{$this->document->ulid}/download");
...
it('returns 404 when the document is in a different business', function () {
    ...
    $response = $this->actingAs($this->member)
        ->get("/documents/{$otherDoc->ulid}/download");
